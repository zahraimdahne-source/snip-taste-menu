...
const ItemModal: React.FC<ItemModalProps> = ({ item, availableSupplements, sectionId, isOpen, onClose, onConfirm }) => {
  const [quantity, setQuantity] = useState(1);
  const [selectedVariant, setSelectedVariant] = useState<'small' | 'large'>('small');
  const [selectedSupplements, setSelectedSupplements] = useState<Supplement[]>([]);
  const [selectedSauce, setSelectedSauce] = useState<string>('');

  const showSauces = ['tex-mex', 'tacos'].includes(sectionId);
  ...
  const handleConfirm = () => {
    const variantLabel = hasVariants ? (selectedVariant === 'small' ? 'Petit' : 'Grand') : undefined;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-snip-bg bg-wood-pattern w-full max-w-md rounded-lg shadow-2xl overflow-hidden border-4 border-white transform scale-100 transition-transform max-h-[90vh] flex flex-col">
        ...
        <div className="p-6 overflow-y-auto">
          ...
          {hasVariants && (
            <div className="mb-6">
              <label className="block font-body font-bold text-gray-700 mb-2 uppercase">Choisir la taille:</label>
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  PETIT
                  <span className="block text-sm opacity-80">{item.prices?.small.toFixed(2)} DH</span>
                </button>
                <button
                ...
                >
                  GRAND
                  <span className="block text-sm opacity-80">{item.prices?.large.toFixed(2)} DH</span>
                </button>
              </div>
            </div>
          )}
          ...
          {showSauces && (
            <div className="mb-6">
              <label className="block font-body font-bold text-gray-700 mb-2 uppercase">Choix de la Sauce:</label>
          ...
          {availableSupplements.length > 0 && (
            <div className="mb-6">
              <label className="block font-body font-bold text-gray-700 mb-2 uppercase">Suppléments (Ajouté séparément):</label>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-5 h-5 rounded border flex items-center justify-center ${
                          isSelected ? 'bg-snip-orange border-snip-orange' : 'border-gray-400 bg-white'
                        }`}>
                          {isSelected && (
                        ...
                        <span className="font-body font-bold text-snip-black uppercase">{supp.name}</span>
                      </div>
                      <span className="font-display text-lg text-snip-orange">+{supp.price.toFixed(2)} DH</span>
                    </div>
                  );
                })}
              </div>
          ...
          {/* Quantity Selector */}
          <div className="mb-8">
            <label className="block font-body font-bold text-gray-700 mb-2 uppercase">Quantité (Article principal):</label>
            <div className="flex items-center justify-center bg-white rounded-lg border-2 border-gray-200 p-2">
              ...
              <span className="flex-1 text-center font-display text-4xl text-snip-black">{quantity}</span>
              <button 
        ...
        <div className="p-6 bg-white border-t-2 border-dashed border-gray-300 shrink-0">
          <div className="flex items-center justify-between mb-4">
            <span className="text-sm text-gray-500 font-body uppercase font-bold">Total (avec suppléments)</span>
            <span className="font-display text-3xl text-snip-black font-bold">{total.toFixed(2)} DH</span>
          </div>
          <button
